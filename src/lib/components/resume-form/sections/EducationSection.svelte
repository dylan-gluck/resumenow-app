<script lang="ts">
	...
	function saveEducation() {
		...
		const educationData = {
			...
			relevant_courses: [...(selectedEducation.relevant_courses || [])].filter(
				(c) => c.trim() !== ''
			)
		};
		...
		if (isEditMode && editIndex >= 0) {
			...
			// Add new education
			$formData.education = [...$formData.education, educationData];
		}
		...
		<Sheet.Root bind:open={sheetOpen}>
			...
			<Sheet.Content side="right">
				<Sheet.Header>
					...
					<Sheet.Description
						>{isEditMode ? 'Update' : 'Add'} details about your education history.</Sheet.Description
					>
				</Sheet.Header>
						...
						<div class="space-y-2">
							...
							{#each selectedEducation.honors as honor, i}
								<div class="flex items-center gap-2">
									<Input bind:value={selectedEducation.honors[i]} placeholder={`Honor ${i + 1}`} />
									{#if i === selectedEducation.honors.length - 1}
										<Button
											variant="outline"
											size="sm"
											onclick={() => {
												selectedEducation.honors = [...selectedEducation.honors, ''];
											}}>+</Button
										>
									{/if}
								</div>
				...
				<Sheet.Footer>
					<div class="flex w-full justify-end gap-2">
						...
						<Sheet.Close class={buttonVariants()} onclick={saveEducation}>
