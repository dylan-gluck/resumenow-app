<script lang="ts">
	...
	function deleteWorkExperience(index: number) {
		...
		$formData.work_experience = $formData.work_experience.filter(
	...
	function saveWorkExperience() {
		...
		const experienceData = {
			...
			responsibilities: [...(selectedWorkExperience.responsibilities || [])].filter(
				(r) => r.trim() !== ''
			),
		...
		if (isEditMode && editIndex >= 0) {
			...
			$formData.work_experience = $formData.work_experience.map(
				(experience: WorkExperience, i: number) => {
					if (i === editIndex) {
			...
			$formData.work_experience = [...$formData.work_experience, experienceData];
		}
		...
		<Sheet.Root bind:open={sheetOpen}>
			...
			<Sheet.Content side="right">
				<Sheet.Header>
					...
					<Sheet.Description
						>{isEditMode ? 'Update' : 'Add'} details about your work history.</Sheet.Description
					>
				</Sheet.Header>
				...
				<Sheet.Footer>
					<div class="flex w-full justify-end gap-2">
						...
						<Sheet.Close class={buttonVariants()} onclick={saveWorkExperience}>
